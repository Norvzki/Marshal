...
console.log("[Gemini] popup.js loaded ✅");

const GEMINI_API_KEY = "AIzaSyBJDBMGMrTeGkLAbqmH3quDg-GbzdH_wqM";
const gemini = new GeminiClient(GEMINI_API_KEY);

document.addEventListener("DOMContentLoaded", () => {
  console.log("[Gemini] DOMContentLoaded fired");
  
  const gwaBtn = document.getElementById("gwaBtn");
  if (!gwaBtn) {
    console.warn("[Gemini] gwaBtn not found in DOM.");
    return;
  }

  // Attach only ONE click listener
  gwaBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (aiBox) {
        aiBox.innerHTML = `<h3>AI Study Advice 🤖</h3><p>${aiResponse}</p>`;
      } else {
        // Create box if it doesn't exist
        const newBox = document.createElement("div");
        newBox.id = "aiSuggestionBox";
        newBox.className = "ai-box";
        newBox.innerHTML = `<h3>AI Study Advice 🤖</h3><p>${aiResponse}</p>`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Load home page AI advice if on home page
  const currentPage = document.querySelector(".page:not(.hidden)");
  if (currentPage?.id === "homePage") {
    console.log("[Gemini] Loading home page AI advice");
    // Delay slightly to ensure tasks are loaded first
    setTimeout(() => loadHomePageAIAdvice(), 1500);
  }
  ...
  // Attach only ONE click listener for GWA page
  gwaBtn.addEventListener("click", async () => {
    console.log("[Gemini] GWA button clicked");
    ...
    // After GWA loads, ask Gemini for feedback
    const gwaText = document.getElementById("gwaDisplay")?.textContent;
    if (!gwaText || gwaText === "--") {
      console.warn("[Gemini] No valid GWA found, skipping AI request.");
      return;
    }

    const prompt = `My current GWA is ${gwaText}. Based on this GWA, suggest 3 brief ways to improve my academic performance. Keep it 3 sentences and be encouraging. Seperate each sentences by \n's`;

    try {
      console.log("[Gemini] Sending prompt:", prompt);
      const aiBox = document.getElementById("aiSuggestionBox");
      if (aiBox) aiBox.innerHTML = `<h3>AI Study Advice 🤖</h3><p>Thinking...</p>`;

      console.log("[Gemini] Sending request...");
      const aiResponse = await gemini.generate(prompt);
      console.log("[Gemini] Raw AI response:", aiResponse);

      if (aiBox) {
        aiBox.innerHTML = `<h3>AI Study Advice 🤖</h3><p>${aiResponse}</p>`;
      } else {
        // If the box doesn't exist yet, create it
        const newBox = document.createElement("div");
        newBox.id = "aiSuggestionBox";
        newBox.className = "ai-box";
        newBox.innerHTML = `<h3>AI Study Advice 🤖</h3><p>${aiResponse}</p>`;
      ...
      console.error("[Gemini] Error:", err);
      const aiBox = document.getElementById("aiSuggestionBox");
      if (aiBox) aiBox.innerHTML = "<p>❌ Error loading AI suggestions.</p>";
    }
  });
});
